...
export default class MSDFTextCollection extends GLCollection {
  ...
  constructor(gl: WebGLRenderingContext | WebGL2RenderingContext, options: { fontSize?: number; onReady?: () => void } = {}) {
    ...
    void fetch(`${fontPath}/Roboto.json`, { mode: 'cors' })
      .then((x) => x.json())
      .then((fontInfo) => {
        this.fontInfo = fontInfo
        this.alphabet = new Map()
        fontInfo.chars.forEach((char: FontLetter) => {
  ...
  addText(textInfo: TextInfo) {
    ...
    for (const detail of layout.charDetailsList) {
      ...
      this.add({
        ...
        charSize: [detail.scaledWidth, -detail.scaledHeight],
        texturePosition: [
  ...
  measureText(textInfo: TextInfo) {
    if (!this.isReady && (!this.fontInfo || !this.alphabet)) {
      ...
      return {
        ...
        scale: 0,
        currentWorldFontSize: fallbackSize,
      }
    }
    if (textInfo.text === undefined) {
      return {
        ...
        error: 'Text not defined',
      }
    }

    const layout = this._calculateTextLayout(textInfo)
    if (layout.error && layout.error !== 'Font info not fully available') {
      // Allow "Font info not fully available" if isReady was false but some info existed
      ...
      return {
        ...
        currentWorldFontSize: layout.currentWorldFontSize || textInfo.fontSize || this.fontSize,
      }
    }
  ...
  _calculateTextLayout(textInfo: TextInfo) {
    ...
    if (text === undefined || text === null) {
      return {
        /* ... error structure ... */ error: 'Text not defined',
        charDetailsList: [],
        overallWidth: 0,
        minWorldY: y,
        maxWorldY: y,
    ...
    const desiredScreenHeight = textInfo.fontSize || this.fontSize
    ...
    const viewportHeight = dc.height
    ...
    if (charDetailsList.length === 0) {
      // If text was empty or all chars missing
