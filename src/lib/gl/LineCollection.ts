...
export default class LineCollection extends GLCollection {
  width: number
  constructor(gl: WebGLRenderingContext | WebGL2RenderingContext, options: { width?: number } = {}) {
  ...
  draw(_: WebGLRenderingContext, drawContext: DrawContext) {
    if (!this.uniforms) {
      this.uniforms = {
  ...
  forEachLine(cb: (arg0: { x: number; y: number; z: number; color: number }, arg1: { x: number; y: number; z: number; color: number }) => void) {
    const count = this.program.getCount()
    for (let i = 0; i < count; ++i) {
      const vertex = this.program.get(i)
      const from = { x: vertex.from[0], y: vertex.from[1], z: vertex.from[2], color: vertex.color }
      const to = { x: vertex.to[0], y: vertex.to[1], z: vertex.to[2], color: vertex.color }
      cb(from, to)
    }
  }

  getLineColor(from: { color: number }) {
    const count = this.program.getCount()
    const c = from ? from.color : count > 0 ? this.program.get(0).color : 0xffffffff

    return [(c >> 24) & (0xff / 255), (c >> 16) & (0xff / 255), (c >> 8) & (0xff / 255), (c >> 0) & (0xff / 255)]
  }
}
