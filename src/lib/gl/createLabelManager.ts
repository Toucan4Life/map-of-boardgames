import type { WglScene } from 'w-gl/src/createScene'
...
export default function createLabelEditor(scene: WglScene) {
  ...
  let visibleRect: { left: number; right: number; top: number; bottom: number; width: number | undefined; height: number | undefined } | undefined
  const allLabelsInfo: Map<string, Label> = new Map()
  const visibleLabels: Set<Label> = new Set()
  let scheduledVisibleLabelsUpdate: number = 0
  let cameraZ: number = scene.getDrawContext().view.position[2]
  const padding = 1 // Padding around labels for overlap checking

  return {
    labels,
    addNodeLabel(node: Label) {
      if (allLabelsInfo.has(node.id)) return
      allLabelsInfo.set(node.id, node)
      updateVisibleLabels()
    },
    redrawLabels,
    setVisibleRect(rect: { left: number; right: number; top: number; bottom: number; width: number; height: number }) {
      visibleRect = {
        left: rect.left,
        right: rect.right,
        top: rect.top,
  ...
  function redrawLabels() {
    labels.clear()
    visibleLabels.forEach((labelInfo: Label) => {
  ...
  function updateVisibleLabelsForReal() {
    ...
    const potentiallyVisible: Label[] = []
  ...
  function isOverlapping(labelInfo: Label, existingLabels: Label[]) {
...
function getTextFromNode(node: Label, cameraZ: number) {
  if (!node.textUI) {
    node.textUI = {
      text: '',
      fontSize: 16,
  ...
  if (node.selectedLevel !== 0 && text.indexOf('/') > -1) {
    text = text.split('/').pop() || 'Could not find /'
  }
