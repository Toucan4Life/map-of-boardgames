...
// import createLayout, { type Layout, type Vector } from 'ngraph.forcelayout'
// import type { DrawContext } from 'w-gl/src/createScene'
// export function createSubgraphViewer(subgraphInfo: { graph: Graph; nodeId: string; onLayoutStatusChange: (arg0: boolean) => void }) {
...
//   let layout: Layout<Graph> | undefined = undefined
//   const graph: Graph<{ selectedLevel: number }> = subgraphInfo.graph
//   let isDisposed = false
//   let layoutSteps = 400
//   let nodes: any, lines: any, labelManager: any
//   let rafHandle: number
//   let lastSelectedNode: string | undefined = undefined
//   const selectedNodeColor = 0xbf2072ff // Selected node color
//   const defaultNodeColor = 0x90f8fcff // Default node color
...
//     let nearestNode: Node<{ IsConnected: boolean }> | undefined = undefined
...
//           x: finalPos.x,
//           y: finalPos.y,
//           id: graph.getNode(nearestNode.id)?.data.id,
...
//     let minDistSq = Infinity
//     let nearestNode: Node<{ IsConnected: boolean; ui: { size: number } }> | undefined = undefined
...
//       updateNodeAppearance(neighborNode.id.toString(), 0xe56aaaff) // Highlight color for neighbors
...
//   function updateNodeAppearance(nodeId: string, color: number) {
...
//   function updateLinkAppearance(link: Link, color: number) {
//     if (link && link.ui) {
//       link.ui.color = color
...
//       const point = layout.getNodePosition(node.id)
...
//   function initializeNodeUI(node: Node, point: { x: number; y: number; z: number }) {
...
//   function initializeLinkUI(link: Link, from: Vector, to: Vector) {
//     if (link.ui) return // already initialized
//     const line = { from: [from.x, from.y, from.z || 0], to: [to.x, to.y, to.z || 0], color: 0xffffff10 }
//     link.ui = line
...
//     const willStop = layoutSteps - 1 === 0
//     if (layoutSteps > 0) {
//       layoutSteps -= 1
//       layout.step()
...
// function getVisibleRectFromDrawContext(drawContext: DrawContext) {
