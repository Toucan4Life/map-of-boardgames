...
/**
 * Explores a repository's local network across group boundaries via BFS, downloading each
 * group's graph as needed (via `downloadGroupGraph`'s cache) and keeping only the top-25
 * strongest links per node, up to `depth` hops from `repositoryName`.
 */
